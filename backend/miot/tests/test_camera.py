...
async def test_miot_camera_async(
    ...
    async def on_raw_video_async(
        did: str, data: bytes, ts: int, seq: int, channel: int
    ):
        _LOGGER.info(
    ...
    async def on_raw_audio_async(
        did: str, data: bytes, ts: int, seq: int, channel: int
    ):
        _LOGGER.info(
            "on_raw_audio: %s, %d, %d, %d, %d", did, channel, len(data), ts, seq
        )
    ...
    async def on_decode_jpg_async(did: str, data: bytes, ts: int, channel: int):
        ...
        async with aiofiles.open(
            os.path.join(image_path, f"./camera_jpg_{did}_{channel}.jpg"), mode="wb"
        ) as f:
            await f.write(data)
...
async def test_miot_camera_with_destroy_async(
    ...
    for _ in range(3):
        ...
        for info in list(cameras.values()):
            ...
            await camera_ins.register_status_changed_async(
                callback=on_status_changed_async
            )
            ...
            async def on_decode_jpg_async(did: str, data: bytes, ts: int, channel: int):
                ...
                async with aiofiles.open(
                    os.path.join(image_path, f"./camera_jpg_{did}_{channel}.jpg"),
                    mode="wb",
                ) as f:
                    await f.write(data)

            await camera_ins.register_decode_jpg_async(
                callback=on_decode_jpg_async, channel=0
            )

            tasks.append(
                camera_ins.start_async(
                    qualities=MIoTCameraVideoQuality.LOW, enable_reconnect=True
                )
            )
        # start cameras.
        ...
        for camera_ins in camera_ins_list:
            tasks.append(
                miot_camera.destroy_camera_async(did=camera_ins.camera_info.did)
            )
        await asyncio.gather(*tasks)
    ...
    for did, camera_info in cameras.items():
        ...
        for channel in range(camera_info["channel_count"]):
            ...
            async def on_decode_jpg_async(did: str, data: bytes, ts: int, channel: int):
                ...
                async with aiofiles.open(
                    f"./camera_jpg_{did}_{channel}.jpg", mode="wb"
                ) as f:
                    await f.write(data)

            await camera_ins.register_decode_jpg_async(
                callback=on_decode_jpg_async, channel=channel
            )
            # await camera_ins.unregister_decode_jpg_async(channel=1)
            ...
            async def on_raw_video_async(
                ...
                _LOGGER.info(
                    "on_raw_video: %s, %d, %d, %d, %d", did, channel, ts, seq, len(data)
                )

            await camera_ins.register_raw_video_async(
                callback=on_raw_video_async, channel=channel
            )
            # await camera_ins.unregister_raw_video_async(channel=1)
...
# ---------------------------------------------------------------------------
# is_camera_model unit tests (pure logic, no network)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_is_camera_model_normal_camera():
    """Camera model not in denylist → True."""
    from miot.camera import is_camera_model

    assert await is_camera_model("xiaomi.camera.082ac1") is True


@pytest.mark.asyncio
async def test_is_camera_model_denied_camera():
    """Camera model in denylist → False."""
    from miot.camera import is_camera_model

    assert await is_camera_model("chuangmi.camera.ipc004b") is False


@pytest.mark.asyncio
async def test_is_camera_model_allowed_wifispeaker():
    """Wifispeaker model in allowlist → True."""
    from miot.camera import is_camera_model

    assert await is_camera_model("xiaomi.wifispeaker.oh11") is True


@pytest.mark.asyncio
async def test_is_camera_model_unknown_wifispeaker():
    """Wifispeaker class in allow_classes but model not in allowlist → False."""
    from miot.camera import is_camera_model

    assert await is_camera_model("xiaomi.wifispeaker.unknown_model") is False


@pytest.mark.asyncio
async def test_is_camera_model_allowed_controller():
    """Controller model in allowlist → True."""
    from miot.camera import is_camera_model

    assert await is_camera_model("xiaomi.controller.oh10p") is True


@pytest.mark.asyncio
async def test_is_camera_model_non_camera_class():
    """Device class not in allow_classes → False."""
    from miot.camera import is_camera_model

    assert await is_camera_model("xiaomi.light.some_model") is False


@pytest.mark.asyncio
async def test_is_camera_model_with_prefetched_info():
    """Passing pre-fetched camera_extra_info skips internal fetch."""
    from miot.camera import get_camera_extra_info, is_camera_model
...
def _bare_camera_instance() -> MIoTCameraInstance:
    """绕过 __init__（依赖 C 库）构造裸实例，只测回调注册簿逻辑。"""
...
@pytest.mark.asyncio
async def test_reg_id_no_collision_after_middle_unregister():
    """复现 reg_id 碰撞时序：A、B 注册 → A 注销 → C 注册。

    旧实现 len+1 会给 C 分配与 B 相同的 id（B 被静默覆盖，B 注销时连带删掉 C）；
    修复后 id 永不重用，B/C 互不影响。
    """
    ins = _bare_camera_instance()

    async def cb_a(did, status): ...
    async def cb_b(did, status): ...
    async def cb_c(did, status): ...

    id_a = await ins.register_status_changed_async(cb_a, multi_reg=True)
    id_b = await ins.register_status_changed_async(cb_b, multi_reg=True)
    await ins.unregister_status_changed_async(id_a)

    id_c = await ins.register_status_changed_async(cb_c, multi_reg=True)
    assert id_c != id_b
    assert ins._callbacks["status"][str(id_b)] is cb_b

    # B 注销不得连带删掉 C
    await ins.unregister_status_changed_async(id_b)
    assert ins._callbacks["status"][str(id_c)] is cb_c


@pytest.mark.asyncio
async def test_reg_id_single_mode_fixed_zero():
    """multi_reg=False 维持单订阅约定：固定 reg_id=0。"""
    ins = _bare_camera_instance()

    async def cb(did, status): ...
