...
from .const import (
    CAMERA_RECONNECT_TIME_MAX,
    CAMERA_RECONNECT_TIME_MIN,
...
_MIOT_CAMERA_ON_RAW_DATA = CFUNCTYPE(
    None, POINTER(_MIoTCameraFrameHeaderC), POINTER(c_uint8)
)
...
class MIoTCameraInstance:
    ...
    async def destroy_async(self) -> None:
        ...
        for key in list(self._callback_refs.keys()):
            if key == "status":
                self._lib_miot_camera.miot_camera_unregister_status_changed(
                    self._c_instance
                )
            elif key.startswith("r"):
                self._lib_miot_camera.miot_camera_unregister_raw_data(
                    self._c_instance, int(key.replace("r", ""))
                )
    ...
    async def start_async(
        self,
        qualities: MIoTCameraVideoQuality
        | List[MIoTCameraVideoQuality] = MIoTCameraVideoQuality.LOW,
        ...
        result: int = self._lib_miot_camera.miot_camera_register_status_changed(
            self._c_instance, c_callback
        )
    ...
    def _alloc_reg_id(self, multi_reg: bool) -> int:
        """multi_reg=False 固定 0（单订阅约定）；multi_reg=True 取实例级递增 id。
        永不重用——旧实现用 len+1 分配，删除中间项后 len 倒退会与现存 id 碰撞，
        新订阅静默覆盖他人回调、对方 unregister 时又把新订阅一起删掉。"""
        if not multi_reg:
            return 0
    ...
    async def register_raw_audio_async(
        self,
        callback: Callable[[str, bytes, int, int, int, MIoTCameraCodec], Coroutine],
        channel: int = 0,
        multi_reg: bool = False,
    ) -> int:
        """Register camera raw audio callback.
        async def on_raw_audio_async(did: str, data: bytes, ts: int, seq: int, channel: int, codec_id: MIoTCameraCodec)
        """
        await self.__update_raw_data_register_status_async(channel=channel)
    ...
    async def unregister_raw_audio_async(
        self, channel: int = 0, reg_id: int = 0
    ) -> None:
    ...
    async def unregister_decode_jpg_async(
        self, channel: int = 0, reg_id: int = 0
    ) -> None:
    ...
    async def unregister_decode_pcm_async(
        self, channel: int = 0, reg_id: int = 0
    ) -> None:
    ...
    async def register_decode_video_frame_async(
        self,
        ...
        async def on_decode_video_frame_async(
            did: str, frame: VideoFrame, ts: int, channel: int,
            recv_unix_ms: int, decoded_unix_ms: int,
        )
        """
        await self.__update_raw_data_register_status_async(channel=channel)
        reg_key: str = f"decode_video_frame.{channel}"
        self._callbacks.setdefault(reg_key, {})
        reg_id: int = self._alloc_reg_id(multi_reg)
        self._callbacks[reg_key][str(reg_id)] = callback
        return reg_id
    ...
    async def unregister_decode_video_frame_async(
        ...
        """Unregister camera decode video frame callback."""
        await self.__update_raw_data_register_status_async(
            channel=channel, is_register=False
        )
        reg_key: str = f"decode_video_frame.{channel}"
    ...
    async def register_decode_audio_frame_async(
        ...
        """Register camera decode audio frame callback.
        async def on_decode_audio_frame_async(
            did: str, frame: AudioFrame, ts: int, channel: int,
            recv_unix_ms: int, decoded_unix_ms: int,
        )
        """
        await self.__update_raw_data_register_status_async(channel=channel)
        reg_key: str = f"decode_audio_frame.{channel}"
        self._callbacks.setdefault(reg_key, {})
        reg_id: int = self._alloc_reg_id(multi_reg)
        self._callbacks[reg_key][str(reg_id)] = callback
        return reg_id

    async def unregister_decode_audio_frame_async(
        self, channel: int = 0, reg_id: int = 0
    ) -> None:
        """Unregister camera decode audio frame callback."""
        await self.__update_raw_data_register_status_async(
            channel=channel, is_register=False
        )
        reg_key: str = f"decode_audio_frame.{channel}"
    ...
    async def __register_raw_data_async(self, channel: int = 0) -> None:
        ...
        result: int = self._lib_miot_camera.miot_camera_register_raw_data(
            self._c_instance, c_callback, channel
        )
        # MUST add to callback refs, otherwise it will be freed.
    ...
    async def __unregister_raw_data_async(self, channel: int = 0) -> None:
        ...
        result: int = self._lib_miot_camera.miot_camera_unregister_raw_data(
            self._c_instance, channel
        )
        self._callback_refs.pop(f"r{channel}", None)
        _LOGGER.info("unregister raw data, %s, %s, %s", self._did, channel, result)

    async def __update_raw_data_register_status_async(
        self, channel: int, is_register: bool = True
    ) -> None:
        ...
        if is_register and reg_key not in self._callback_refs:
            ...
            for cb_prefix in (
                ...
                if len(self._callbacks.get(f"{cb_prefix}.{channel}", {})) > 0:
                    need_unreg = False
                    break
            if need_unreg:
    ...
    async def __try_start_async(self) -> None:
        ...
        result: int = await self._main_loop.run_in_executor(
            None,
            self._lib_miot_camera.miot_camera_start,
            self._c_instance,
            byref(
                _MIoTCameraConfigC(
                    (c_uint8 * ((self.camera_info.channel_count or 1) + 1))(
        ...
        if self._enable_reconnect:
            self._reconnect_timer = self._main_loop.call_later(
                self.__get_try_start_timeout(),
                lambda: self._main_loop.create_task(self.__try_start_async()),
            )
        else:
            _LOGGER.error("camera start failed, %s, %s", self._did, result)
            raise MIoTCameraError(
                f"camera start failed, {self.camera_info.did}, {result}"
            )

    def __get_try_start_timeout(self) -> int:
        self._reconnect_timeout = min(
            self._reconnect_timeout * 2, CAMERA_RECONNECT_TIME_MAX
        )
        _LOGGER.info(
            "get reconnect timeout, %s, %s", self._did, self._reconnect_timeout
        )
    ...
    def __reset_try_start_timeout(self) -> None:
        self._reconnect_timeout = CAMERA_RECONNECT_TIME_MIN
        _LOGGER.info(
            "reset reconnect timeout, %s, %s", self._did, self._reconnect_timeout
        )
    ...
    def __on_status_changed(self, status: int) -> None:
        ...
        for callback in s_callbacks.values():
            asyncio.run_coroutine_threadsafe(
                callback(self._did, camera_status), self._main_loop
            )
    ...
    def __on_raw_data(self, frame_header_ptr: Any, data: bytes) -> None:
        ...
        # Stamp host-side arrival time ASAP — before any queueing or decode
        # work. This is the reference point for separating network latency
        # (device PTS → this moment) from decode latency (this moment →
        ...
        if codec_id in [MIoTCameraCodec.VIDEO_H264, MIoTCameraCodec.VIDEO_H265]:
            ...
            if self._callbacks.get(
                f"decode_jpg.{channel}", None
            ) or self._callbacks.get(f"decode_video_frame.{channel}", None):
            ...
            # raw audio
            if self._callbacks.get(
                f"decode_pcm.{channel}", None
            ) or self._callbacks.get(f"decode_audio_frame.{channel}", None):
            ...
            _LOGGER.error(
                "unknown codec, %s, %s, %s", self._did, codec_id, frame_header.timestamp
            )
        # _LOGGER.info("raw, %s, %s, %s, %s", self._did, channel, frame_header.timestamp, frame_header.sequence)

    async def __on_video_decode_callback(
        self, data: bytes, timestamp: int, channel: int
    ) -> None:
        ...
        for callback in list(v_callbacks.values()):
            asyncio.run_coroutine_threadsafe(
                callback(self._did, data, timestamp, channel), self._main_loop
            )

    async def __on_audio_decode_callback(
        self, data: bytes, timestamp: int, channel: int
    ) -> None:
    ...
    async def __on_video_frame_decode_callback(
        self,
        frame: "VideoFrame",
        timestamp: int,
        channel: int,
        ...
        """On video frame decode callback — emits BGR ndarray plus timing.

        ``recv_unix_ms``/``decoded_unix_ms`` let downstream separate
        network latency (device PTS → host arrival) from decode latency
        (host arrival → decode completion).
        """
        vf_callbacks = self._callbacks.get(f"decode_video_frame.{channel}", {})
        for callback in list(vf_callbacks.values()):
            asyncio.run_coroutine_threadsafe(
                callback(
                    self._did,
                    frame,
    ...
    async def __on_audio_frame_decode_callback(
        self,
        frame: "AudioFrame",
        timestamp: int,
        channel: int,
        ...
        """On audio frame decode callback — emits PCM ndarray plus timing."""
        af_callbacks = self._callbacks.get(f"decode_audio_frame.{channel}", {})
        for callback in list(af_callbacks.values()):
            asyncio.run_coroutine_threadsafe(
                callback(
                    self._did,
                    frame,
...
def _load_dynamic_lib():
    ...
    # miot_camera_start
    lib_miot_camera.miot_camera_start.argtypes = [
        _MIoTCameraInstanceC,
        POINTER(_MIoTCameraConfigC),
    ]
...
class MIoTCamera:
    ...
    def __init__(
        ...
        # Load shared library (idempotent, OS-level only). The paired
        # miot_camera_init / miot_camera_deinit are managed by
        # init_async / deinit_async so the same instance can be reused
    ...
    def __del__(self):
        """Del."""
        # __init__ may raise before these attrs are assigned; guard with getattr.
        if getattr(self, "_lib_miot_camera", None) and getattr(self, "_init_done", False):
            try:
    ...
    async def init_async(
        self, frame_interval: int = 500, enable_hw_accel: bool = False
    ) -> None:
        """Init."""
        if self._init_done:
            return
        ...
        try:
            ...
            # Roll back the log handler so the C library does not retain a
            # pointer to a ctypes callback that may be freed once this
            # instance is GC'd (deinit_async/__del__ skip cleanup while
            # _init_done is False).
            try:
                self._lib_miot_camera.miot_camera_set_log_handler(None)
            except Exception as err:
                _LOGGER.warning(
                    "miot_camera_set_log_handler(None) on init rollback failed: %s",
                    err,
                )
            raise
        self._init_done = True
        _LOGGER.info(
            "miot camera lib version: %s", await self.get_camera_version_async()
        )
        ...
        try:
            for did in list(self._camera_map.keys()):
                try:
                    ...
                    _LOGGER.warning("destroy_camera(%s) failed: %s", did, err)
            self._camera_map.clear()
            try:
                self._lib_miot_camera.miot_camera_set_log_handler(None)
            except Exception as err:
                _LOGGER.warning("miot_camera_set_log_handler(None) failed: %s", err)
            try:
                self._lib_miot_camera.miot_camera_deinit()
            except Exception as err:
                _LOGGER.warning("miot_camera_deinit failed: %s", err)
        finally:
            # Keep self._lib_miot_camera so init_async can call miot_camera_init
            # on the same instance again. Always clear _init_done so that
            # __del__ won't attempt a second miot_camera_deinit after a partial
            # failure here.
            self._init_done = False
    ...
    async def update_access_token_async(self, access_token: str) -> None:
        """Update access token."""
        self._access_token = access_token
        self._lib_miot_camera.miot_camera_update_access_token(
            self._access_token.encode("utf-8")
        )
    ...
    async def create_camera_async(
        ...
        camera: MIoTCameraInfo = (
            MIoTCameraInfo(**camera_info)
            if isinstance(camera_info, Dict)
            else camera_info.model_copy()
        )
        did: str = camera.did
    ...
    async def get_camera_version_async(self) -> str:
        """Get camera version."""
        result: bytes = await self._main_loop.run_in_executor(
            None, self._lib_miot_camera.miot_camera_version
        )
    ...
    async def register_status_changed_async(
        ...
        if did not in self._camera_map:
            _LOGGER.error("camera not found, %s", did)
            raise MIoTCameraError(f"camera not found, {did}")
        return await self._camera_map[did].register_status_changed_async(
            callback=callback, multi_reg=multi_reg
        )
    ...
    async def unregister_status_changed_async(self, did: str, reg_id: int = 0) -> None:
        ...
        if did not in self._camera_map:
            ...
            raise MIoTCameraError(f"camera not found, {did}")
        return await self._camera_map[did].unregister_status_changed_async(
            reg_id=reg_id
        )
    ...
    async def unregister_raw_video_async(
        ...
        return await self._camera_map[did].unregister_raw_video_async(
            channel=channel, reg_id=reg_id
        )
    ...
    async def unregister_raw_audio_async(
        ...
        return await self._camera_map[did].unregister_raw_audio_async(
            channel=channel, reg_id=reg_id
        )
    ...
    async def unregister_decode_jpg_async(
        ...
        if channel < 0 or channel >= self._camera_map[did].camera_info.channel_count:
            _LOGGER.error("invalid channel, %s, %s", did, channel)
            raise MIoTCameraError(f"invalid channel, {did}, {channel}")
        return await self._camera_map[did].unregister_decode_jpg_async(
            channel=channel, reg_id=reg_id
        )
    ...
    async def unregister_decode_pcm_async(
        ...
        if channel < 0 or channel >= self._camera_map[did].camera_info.channel_count:
            _LOGGER.error("invalid channel, %s, %s", did, channel)
            raise MIoTCameraError(f"invalid channel, {did}, {channel}")
        return await self._camera_map[did].unregister_decode_pcm_async(
            channel=channel, reg_id=reg_id
        )
    ...
    async def unregister_decode_video_frame_async(
        ...
        """Unregister decode video frame."""
        if did not in self._camera_map:
            raise MIoTCameraError(f"camera not found, {did}")
        if channel < 0 or channel >= self._camera_map[did].camera_info.channel_count:
            raise MIoTCameraError(f"invalid channel, {did}, {channel}")
        return await self._camera_map[did].unregister_decode_video_frame_async(
            channel=channel, reg_id=reg_id
        )
    ...
    async def register_decode_audio_frame_async(
        ...
        if channel < 0 or channel >= self._camera_map[did].camera_info.channel_count:
            raise MIoTCameraError(f"invalid channel, {did}, {channel}")
        return await self._camera_map[did].register_decode_audio_frame_async(
            callback=callback, channel=channel, multi_reg=multi_reg
        )
    ...
    async def unregister_decode_audio_frame_async(
        ...
        """Unregister decode audio frame."""
        if did not in self._camera_map:
            raise MIoTCameraError(f"camera not found, {did}")
        if channel < 0 or channel >= self._camera_map[did].camera_info.channel_count:
            raise MIoTCameraError(f"invalid channel, {did}, {channel}")
        return await self._camera_map[did].unregister_decode_audio_frame_async(
            channel=channel, reg_id=reg_id
        )
...
async def is_camera_model(
    model: str,
    camera_extra_info: MIoTCameraExtraInfo | None = None,
) -> bool:
    """Check if a device model belongs to a camera class.

    Shared logic used by both MIoTClient.get_cameras_async (iterate all
    devices) and MiotProxy.is_camera (single-did lookup).

    Args:
        model: Device model string (e.g. "chuangmi.camera.068ac1").
        camera_extra_info: Pre-fetched extra info. When None (default),
            fetches internally via get_camera_extra_info() (cached 10min).
    """
    if camera_extra_info is None:
        camera_extra_info = await get_camera_extra_info()
    ...
    if device_class in camera_extra_info.denylist:
        return model not in camera_extra_info.denylist[device_class]
    if device_class in camera_extra_info.allowlist:
        return model in camera_extra_info.allowlist[device_class]
    return False
