...
- ONNX Runtime 推理线程自适应低核机器：4 核及以下默认把 intra-op（算子内部并行）收敛到 1，inter-op（算子之间并行）固定为 1，避免检测模型和 ReID 模型在 NAS 上各自开满线程池互相抢 CPU。模型、输入、阈值和输出不变，只收敛调度并行度。
- OpenCV 图像处理线程也会在低核机器上收敛，避免缩放、转灰度和帧差计算与 ONNX 推理同时抢满 CPU。
- Linux server 进程默认启用 CPU 亲和性预算：Miloco 后端只绑定到当前可用核心数的一半，4 核 NAS 即 2 核。这样用更长的处理时间换取“不吃爆宿主 CPU”的硬约束；可通过 `MILOCO_CPU_AFFINITY_CORES=off` 关闭，或指定核心数。
...
- 首页实时画面默认改为轻量 JPEG snapshot（快照，一张最近画面图片）：后端从感知缓冲读取最近 BGR 帧，小卡片缩到 640px 宽，放大预览缩到 1280px 宽后返回 JPEG。这样小卡片和放大弹窗都不再常驻 H.264 iframe 预览流，避免为了一个展示画面启动额外本地转码。实时直播应作为后续显式按钮或高配模式能力，不再是低配 NAS 的默认预览路径。
...
6. 面板预览不要默认走重视频链路
   对低配 NAS 来说，首页小卡片和放大弹窗的价值是“确认画面源可见”，不是低延迟直播。默认用感知快照即可；实时视频只在用户主动选择高配/直播能力时开启。验证时不能只看 `/api/miot/scope/cameras` 的 `connected=true`，还要打开浏览器确认小卡片和放大态实际显示图片。
...
3. 定时采样
   优先用 `./manage.sh perf-probe --duration 120 --interval 5` 采 Miloco 后端进程的区间 CPU 和 RSS。这个脚本默认只读，不保存 token；需要 A/B 时显式传 `--profile default-high --apply`，它会备份 `config.json`、重启、采样、恢复并再次重启。`SUMMARY_JSON.trace_summary.stage` 会带最近 trace 的阶段耗时，`trace_summary.omni_video` 会带 remux/reencode 统计；引擎状态和更细的历史 trace 再用性能页或只读 DB 补充。
...
| 第一轮-参数化 | 将摄像头音频感知暴露为性能参数 `camera.enable_audio_perception`；默认关闭，开启后重启时摄像头实例用 `enable_audio=true` 并订阅解码音频流 | `pytest backend/miloco/tests/admin/test_performance_tuning.py backend/miloco/tests/perception/test_camera_adapter_reconnect.py::TestConnectDeviceManagerMissing::test_audio_subscription_enabled_by_config backend/miloco/tests/test_miot_filter_and_cameras.py::test_create_camera_img_manager_passes_video_quality_override backend/miloco/tests/test_miot_filter_and_cameras.py::test_create_camera_img_manager_passes_audio_perception_setting backend/miloco/tests/test_settings.py::test_pydantic_matches_settings_schema -q`：17 passed | NAS 只读健康确认：`/health` 为 `ok`，Miloco 约 17% CPU、342MB RSS，无 `/tmp/codex-miloco-audio-*` 残留；因当前 WebSocket 传输通道在补丁上传时不稳定，本轮未强行热更新运行包 | 这是可选能力开关，不再写死“当前仓库永远无音频感知”。低配模式仍关闭它以节省拉流、音频解码、声音 gate 和 VAD CPU；打开后声音触发/音频 Omni 可用，但会增加本地压力 |
...
| 第一轮-质量保护 | 主动查询路径增加 H.265 remux 空回答自动回退：如果 H.265 直传已成功但 Omni 解析结果为空，则临时去掉原始包，复用同一窗口的解码帧走 BGR 再编码再问一次；同时记录 `h265_empty_retry=1` | `uv run pytest tests/perception/engine/test_pipeline.py::test_query_empty_h265_remux_retries_with_reencode tests/perception/engine/test_pipeline.py::test_query_omni_error_preserves_video_encode_stats -q`：2 passed；`uv run pytest ...test_prompt_builder.py` 定向通过；ruff 通过；补统计字段后相关 pipeline/stats 测试 5 passed、ruff 通过 | NAS 热补丁 `pipeline.py` 并重启后 `/health` 恢复 ok；当前低配态主动查询 21.64 秒返回正常中文画面描述；22 笔 1 秒采样：CPU 峰值 26.7%，RSS 峰值 336.7MB；统计仍是 `remux_success=1`、`reencode=0`、`input_packets=1147`、`output_bytes=1476941`，说明正常 H.265 直传路径未被破坏；传输服务和 `/tmp/codex-*` 已清理 | 这是第一轮“不降低质量”的护栏：H.265 直传成功时省 CPU；若再遇到空回答，则自动退回旧的再编码路径，优先保证用户能拿到答案。`h265_empty_retry` 让后续诊断能看出“这次再编码是因为空回答兜底”，而不是误判为原始包不可用。下一步仍要用长窗口/运动场景专门触发一次空回答，验证实际回退分支 |
| 第一轮-可视化部署 | 构建并热部署新版性能页静态资源，让“云端上传视频”卡片显示 `H.265 空回答重试` 和 `h265_empty_retry` | `pnpm run typecheck` 通过；`pnpm run build` 通过，产物为 `assets/index-C8zZKqLu.js`、`assets/index-Bh52g5ga.css` | NAS 仅替换包内静态目录，不重启后端；备份 `/data/miloco/static-backup-codex-20260703-124600.tar.gz`；HTTP 首页已引用 `index-C8zZKqLu.js` / `index-Bh52g5ga.css`；新 JS 内可 grep 到 `H.265 空回答重试` 和 `h265_empty_retry`；`/health` 为 ok，进程约 `21.9% CPU / 353MB RSS`；传输服务和 `/tmp/codex-*` 已清理 | 这不改变运行负载，但让小白用户能在性能页区分“原始包复用成功”“H.265 主动质量兜底”和“H.265 空回答后自动重试”，避免只看到再编码次数而误判原因 |
...
| 运维固化 | 补齐 CLI supervisor `env -u` 解锁列表：新增 `MILOCO_CAMERA__ENABLE_AUDIO_PERCEPTION` 和此前漏掉的 `MILOCO_PERCEPTION__ENGINE__OMNI__ALLOW_H265_REMUX`，并用测试从 `CONFIG_SPECS` 自动推导全集做比对 | `cd cli && uv run pytest tests/test_commands.py::test_service_supervisor_command_unsets_perf_env -q`：1 passed；`../backend/.venv/bin/ruff check src/miloco_cli/commands/service.py tests/test_commands.py` 通过 | NAS 只读确认当前低配运行态健康：`/health` 为 `ok`，配置为 `LOW/5000ms/1 FPS/mock`，进程约 17.3% CPU、316MB RSS，无 `/tmp/codex-*` 残留；本改动未热改 NAS | 防止性能页参数重蹈 `frame_interval` 的环境变量覆盖问题；以后新增性能页参数时，CLI 测试会因全集不一致而失败，避免再漏解锁项 |
| 运维固化 | 新增 `./manage.sh perf-probe`，封装 NAS 容器内 CPU/RAM 采样；默认只读，`--profile default-high --apply` 才会备份配置、临时切 profile、重启、采样、恢复并再次重启 | `python3 -m py_compile nas/docker/perf_probe.py` 通过；`bash -n nas/docker/manage.sh` 通过；`ruff check nas/docker/perf_probe.py` 通过；AST 对比确认 `low-power` profile 与后端 `LOW_POWER_SAFE_MODE_VALUES` 一致 | 当前 NAS 镜像未重建，临时传入脚本到 `/tmp` 做只读 10 秒验证：5 笔采样，CPU 峰值 10.0%、平均 9.1%，RSS 峰值 321.0MB；`/health` 为 `ok`，临时脚本、本机 HTTP 服务和 `/tmp/codex-*` 已清理 | 后续每轮 NAS 复验有了可重复入口，避免继续手搓长 WebSocket 命令；脚本按 `/proc` 区间 jiffies 计算 CPU，更适合看运行期峰值 |
| 运维固化 | `perf-probe` 增加最近 trace 聚合：阶段耗时 `stage`、窗口丢弃/溢出、Omni 视频构造 `remux/reencode/H.265 retry`，只输出聚合数字和最新 trace id，不输出原始 timing detail | `python3 -m py_compile nas/docker/perf_probe.py` 通过；`ruff check nas/docker/perf_probe.py` 通过；临时 SQLite 烟测确认 `decode_ms` 和 `omni_video_*` 可聚合 | 当前 NAS 镜像未重建，临时传入新版脚本做只读 10 秒验证：CPU 峰值 11.5%、RSS 峰值 323.7MB；最近 10 分钟 11 条 trace 全部 gate 跳过，`gate_video_ms` p95 约 477.7ms，`identity_ms/omni_ms` 样本为 0，`raw_window_packets_total=9877`；`/health` 为 `ok`，临时脚本和本机 HTTP 服务已清理 | 现在一条采样命令能同时回答“整体有没有超 50%”和“最近窗口主要耗在哪段”。本轮样本说明当前低配静态场景的 CPU 主要来自 Gate 画面变化检测，而不是身份识别或 Omni |
| 第一轮-默认质量抽样 | 在同一 NAS 上临时切到默认高质量组合：`HIGH/1000ms/max_cache_images=6`、`input.fps=3`、`period_sec=4`、`deep_sort fast`、`identity_engine.enabled=true`、音频感知关闭；测试前备份 `config.json`，测试后恢复低配配置并重启 | 无代码变更 | 16 笔、每 5 秒一次的短采样：CPU 从 140% 逐步降到 122%，RSS 峰值约 536MB，低于 4 核宿主 200% CPU 预算和约 3905.5MB RAM 预算；随后恢复 `LOW/5000ms/1 FPS/mock`，`/health` 为 `ok`，预算接口恢复为 CPU 14.2%、RSS 310MB，`/tmp/codex-*` 与采样进程已清理 | 说明当前 live NAS 在默认高质量短时静态窗口下没有立即超预算，但样本不足以替代 5 分钟、多路、移动和规则触发复验。重启后预算接口可能短暂读到启动窗口尖峰，必须用 `ps` 连续直读确认稳态，避免把瞬时异常当作服务崩溃 |
| 第一轮-默认质量反证 | 用新版 `perf-probe` 临时切到同一默认高质量组合并采样 3 分钟，`trace_scope=run` 只看本次测试产生的 trace；采样后自动恢复低配配置并重启 | `perf-probe` 本地语法、ruff、trace scope SQLite 烟测通过 | 36 笔、每 5 秒一次：CPU 峰值 300.9%、平均 127.5%，超过 4 核宿主 200% 预算；RSS 峰值 608.2MB，远低于内存预算。37 条本轮 trace 中 36 条 gate 跳过，1 条进入 identity/omni：`identity_ms=18369.8ms`、`omni_ms=5317.8ms`、`decode_ms p95=100.0ms`、`gate_video_ms p95=176.3ms`；测试后恢复 `LOW/5000ms/1 FPS/mock`，`/health` 为 `ok`，临时脚本和输出已清理 | 默认高质量仍未达到“最高峰 CPU < 50% 宿主”的目标；峰值集中在重启后首个 identity/omni 窗口，下一步第一轮无损优化必须削 identity 首窗峰值，不能只看稳态均值 |
...
| 第一轮-CPU预算硬约束 | 修正 `human_reid_skip_windows` 运行期语义，让它按真实窗口长度和 FPS 生效；4 核及以下 ONNX/OpenCV 都收敛到 1 线程；Linux server 默认把 Miloco 后端 CPU 亲和性限制到可用核心数的一半 | `pytest tests/perception/test_ort_utils.py tests/perception/engine/identity/test_deep_sort_v12.py::TestDeepSortConfigDC tests/test_bootstrap_thread_limits.py -q`：14 passed；定向 ruff 通过；`perf-probe` py_compile 通过 | NAS 热补丁后 `taskset` 确认后端绑定 CPU `0,1`；临时切 `HIGH/1000ms/3 FPS/deep_sort fast` 采样 3 分钟，36 笔每 5 秒一次：CPU 峰值 199.9%、平均 134.6%，低于 4 核宿主 200% 预算；RSS 峰值 568.2MB，远低于 3905.5MB 内存预算；本轮 trace 中 `identity_ms=33611.9ms`、`omni_ms=4993.2ms`，说明质量不降但处理时间变长；测试后恢复 `LOW/5000ms/1 FPS/mock`，`/health` 为 `ok` | 第一轮在当前一路 NAS 场景下首次满足“最高峰 CPU/RAM 均不超过宿主 50%”。这是硬预算方案：不改画面质量、模型、阈值和窗口内容，但允许低配机器用更长本地处理时间换不抢爆宿主 |
...
针对这个默认质量 CPU 峰值，源码进一步收敛 ONNX Runtime 线程：低核 NAS 上 detector（目标检测模型）和 ReID（人体特征模型）不再各自开多线程池，而是 4 核机器默认 intra-op=1、inter-op=1。这里的 intra-op 指单个算子内部并行，inter-op 指多个算子之间并行；这只改变线程调度，不改变模型本身和识别输出。
...
最新一轮 NAS 验证改用硬预算控制：OpenCV 图像处理也按低核机器收敛线程，`human_reid_skip_windows` 修正为按真实窗口长度生效，Linux server 进程默认绑定到当前可用核心数的一半。4 核 NAS 上 `taskset` 显示 Miloco 后端只运行在 CPU `0,1`。在同样 `HIGH/1000ms/3 FPS/deep_sort fast` 默认高质量组合下采样 3 分钟，36 笔 5 秒样本的 CPU 峰值为 199.9%、平均 134.6%，RSS 峰值 568.2MB；采样后恢复 `LOW/5000ms/1 FPS/mock`，健康接口为 `ok`。代价是 `identity_ms` 从约 18-33 秒区间继续保持在慢路径，说明低配机器会以更长本地处理时间换取不抢爆宿主 CPU；画面质量、模型、阈值和 Omni 输入内容没有降级。

2026-07-03 又在当前 live NAS 上做了一次短时恢复验证：先确认性能参数没有被 `MILOCO_*` 环境变量锁定，再临时切到 `HIGH/1000ms/3 FPS/deep_sort fast`，16 笔 5 秒采样中 CPU 从 140% 下降到 122%，RSS 峰值约 536MB。测试后恢复 `LOW/5000ms/1 FPS/mock` 并重启，健康接口为 `ok`，连续 `ps` 直读显示 CPU 从 26.1% 降到 22.1%，RSS 约 319-325MB，随后预算接口恢复为 CPU 14.2%、RSS 310MB。注意：刚重启后 `/admin/performance-budget` 曾短暂报出 736.3% CPU，这类读数可能来自启动窗口或采样时间差，必须用进程级连续采样复核，不应直接判定服务已经被带崩。
...
## 当前 goal 进度快照（2026-07-04）

当前 `/goal` 是暂停态，不是完成态。完成度可以按“诊断和热验证已经大量完成，但正式固化和验收未完成”理解。

已完成的部分：

- 已弄清主要运行机制：摄像头只拉一次流，本地解码帧供 Gate（画面变化检测）、Identity（身份识别与跟踪）使用，原始压缩包优先供 Omni 上传 remux（转封装，只换容器，不重新压缩画面）使用；不可用时才 reencode（再编码，把图片帧重新压成视频）。
- 已证明本地压力不会因为配置了 LLM API 就消失。LLM API 只减少本地“理解”压力，本地仍要承担拉流、解码、变化检测、身份识别、视频构造和上传前准备。
- 已在 NAS 单路桌面摄像头上验证过多组配置：第二轮 LOW 低配方案有明显余量；第一轮默认质量通过线程收敛和 CPU 亲和性能贴近 4 核宿主 50% CPU 预算。
- 已把“小白可读”的方向明确：性能页必须用中文解释 CPU/RAM、饼图、柱状图、参数作用、参数范围和改动影响，英文参数名只作为低权重注释。

未完成的部分：

- 当前 NAS 容器被重建后，旧容器内的性能页热改丢失，页面回到镜像默认性能报告页。这说明改动没有完整固化到镜像或发布包。
- 当前感知引擎存在配置和运行时代码不一致风险，典型错误是 `OmniConfig` 收到未知参数。这个问题必须先解决，否则无法进行可信性能验收。
- OpenClaw Miloco 插件需要重新核验。容器重建会丢失旧容器内临时安装或热改的内容。
- 第一轮“不降低质量”的最终验收还缺多路摄像头、有人移动、夜间、规则触发和长时间运行样本。
- 第二轮“约 80% 服务质量”的低配方案还缺真实家庭场景可接受性验证。

下一步必须先做固化，再做复验：

```mermaid
flowchart TD
  A["修复感知引擎启动异常"] --> B["把性能中心和配置 API 固化进构建产物"]
  B --> C["重新部署 NAS 镜像或发布包"]
  C --> D["确认容器重建后页面仍是低配资源中心"]
  D --> E["单路摄像头 5-10 分钟只读采样"]
  E --> F["多路、移动、夜间、规则触发验收"]
```

状态页见 `docs/nas/low-config-performance-current-status.md`。

## 绿联 post/ports 模式与摄像头感知结论（2026-07-04）

绿联 NAS 上不能把 Web 可访问性和摄像头感知可用性混为一件事。`ports: 1810/18789` 的 bridge 模式能让 Miloco 面板和 OpenClaw 页面继续打开，也方便绿联面板生成快速访问/反代入口；但摄像头视频链路依赖局域网内的直连/PPCS/UDP 通道，bridge 容器里会出现只看得到网页、看不到摄像头帧的状态。

现场证据：

- bridge 容器内路由是 `default via 172.21.0.1 dev eth0`，摄像头列表长期 `connected=false`，感知引擎 `active_sources=[]`。
- 同一数据目录恢复为 host 网络后，容器路由回到宿主 `192.168.31.225`；现场最终只开启一路 `主卧 电脑桌上`，该摄像头恢复 `in_use=true`、`connected=true`，感知引擎出现 active source。
- OpenClaw/Miloco 内部 post 回调地址不是这次摄像头故障根因；真正根因是 Docker 网络从 host 改成 bridge/ports。
