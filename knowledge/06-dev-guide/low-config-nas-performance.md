...
3. 定时采样
   优先用 `./manage.sh perf-probe --duration 120 --interval 5` 采 Miloco 后端进程的区间 CPU 和 RSS。这个脚本默认只读，不保存 token；需要 A/B 时显式传 `--profile default-high --apply`，它会备份 `config.json`、重启、采样、恢复并再次重启。阶段耗时、引擎状态和历史 trace 再用性能页或只读 DB 补充。
