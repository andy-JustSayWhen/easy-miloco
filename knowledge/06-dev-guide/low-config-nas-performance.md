...
| 观测-上传视频 | 新增 `/api/stats?metric=omni_video_summary` 和性能页“云端上传视频”卡片，显示最近 Omni 上传前是 remux（复用原始视频包）还是 reencode（重新编码），并显示 P95 上传体积、H.265 质量兜底次数；随后补充 `raw_window_packets/raw_keyframes`，区分“原始包不存在”和“原始包存在但不可安全复用” | `env MILOCO_SERVER__TOKEN= uv run pytest tests/observability/test_stats.py -q`：17 passed；定向 ruff 通过；`pnpm run typecheck` 与 `pnpm run build` 通过 | NAS 热补丁 `stats.py` 和 Web 静态资源后重启后端，`/health` 为 ok；授权访问新接口返回 `sample_count=56`、`remux_success_count=0`、`reencode_count=56`、`h265_remux_skipped_count=31`、`output_bytes_p95≈2.25MB`、最近样本 `mode=reencode`；进一步只读查库确认近期样本 `raw_encoded_video_window_packets≈1200`、`raw_encoded_video_keyframes=0`、`input_packets=0`，说明原始压缩包已经在同一次拉流里存在，但缺可用关键帧，不能安全 remux；首页引用 `index-BhVgjgn8.js` / `index-ntSZQXmK.css`；清理本地 HTTP 服务、传输包和 NAS `/tmp/codex-*` | 这不直接降 CPU，但把“为什么配了 LLM API 本地仍吃 CPU”变成可见数据：如果是 reencode，CPU 压力来自上传前重新压缩；如果是 remux，则说明复用了同一次拉流资产；如果窗口原始包>0但关键帧=0，则下一步要查关键帧标记/GOP，而不是误判为又拉了一次流 |
...
3. H.265（HEVC，另一种压缩格式）当前不走 remux。NAS 实测本地能封装，但 Omni 返回空答案；为保证回答质量，H.265 只保留 metadata（时间戳、序号、关键帧标记等），不长期保留大块 payload（视频包内容），上传时走 reencode。
